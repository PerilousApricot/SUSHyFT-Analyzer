...
bool SHyFTSelector::operator() ( edm::EventBase const & event, pat::strbitset & ret)
{
    ...
    if (!ignoreCut(triggerIndex_) ) {
        ...
        const edm::TriggerNames & triggerNames = \
                                        event.triggerNames(*triggerResults);

        for(unsigned int nTrig = 0; nTrig < triggerResults->size() && ! passTrig_; ++nTrig) {
            std::string triggerName = triggerNames.triggerName(nTrig);
            std::vector<std::string>::const_iterator triggerIter;
            for(triggerIter = muTriggerList_.begin();
                    triggerIter != muTriggerList_.end();
                    ++triggerIter ) {
                if( (triggerName.find(*triggerIter) == 0) && \
                                        ((*triggerResults)[nTrig].accept()) ) {
                    passTrig_ = 1;
                    break;
                }
            }
    ...
    if ( ignoreCut(pvIndex_) || passPV ) {
        ...
        for ( std::vector<pat::Tau>::const_iterator tauBegin = tauHandle->begin(),
            ...
            if ( (itau->pt() > tauPtMin_) && (fabs(itau->eta()) < tauEtaMax_) ){
                allTaus_.push_back( edm::Ptr<pat::Tau>( tauHandle, itau - tauBegin ) );
                selectedTaus_.push_back( edm::Ptr<pat::Tau>( tauHandle, itau - tauBegin ) );
            }
        }
