...
bool SHyFTSelector::operator() ( edm::EventBase const & event, pat::strbitset & ret)
{
    ...
    if (!ignoreCut(triggerIndex_) ) {
        ...
        event.getByLabel(muTrig_, _triggerResults);

        const edm::TriggerNames & TheTriggerNames = event.triggerNames(*_triggerResults);
        
        for(unsigned int nTrig = 0; nTrig < _triggerResults->size(); ++nTrig) {
            string trigName = TheTriggerNames.triggerName(nTrig);
            std::vector<std::string>::const_iterator TheTriggerPath;
            for(TheTriggerPath = muTriggerList_.begin();
                    TheTriggerPath != muTriggerList_.end();
                    ++TheTriggerPath ) {
                if( (trigName.find(*TheTriggerPath) != string::npos) && \
                        ((*_triggerResults)[nTrig].accept()) ) {
                    passTrig = true;
                }
            }
        }
